...
def run_exploit(offs, interface, stage1, stage2):
    exploit = Exploit(offs, interface, stage1, stage2)
    exploit.run()

def main():
    parser = argparse.ArgumentParser('pppwn.py')
    ...
    # Jalankan skrip di latar belakang
    p = multiprocessing.Process(target=run_exploit, args=(offs, args.interface, stage1, stage2))
    p.start()

    # Tampilkan pesan loading dengan tqdm
    for _ in tqdm(range(100), desc='Executing exploit', ascii=True, dynamic_ncols=True):
        sleep(0.1)

    # Tunggu sampai proses di latar belakang selesai
    p.join()
